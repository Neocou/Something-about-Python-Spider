...
class Sql(object):
    ...
    def addchapter(self,novelid,title,content):
        try:
            cur = self.conn.cursor()
            cur.execute("insert ignore into chapter(novelid,title,content) values(%s,'%s','%s')" %(novelid,title,content))#coding:UTF-8
...
def get_chapter_list(url,novelid):
    ...
    for url_detail,title in chapter_info:
        print(url_detail)
        get_chapter_content(url_detail,novelid,title)

    print('爬取小说编号：%s章节完毕' %(novelid))

#获得每本小说下的章节内容被加入数据库

def get_chapter_content(url_detail,novelid,title):
    chapter_url = url_detail
...
def get_list(sort_id,sort_name):
    ...
    while('true'):
        ...
        if url_list:
            print('开始爬取%s_%s'%(sort_id,i))
            for url in url_list:#coding:UTF-8
